...
def draw_obs(year):
    ...
    dic = {} # {'06':'0728_0600'}
    dic_title = {}  # {'06':'0600UTC'}
    for i in ftime:
        key = i[-4:-2]
        dic.update({key:i})  # 生成时间的字典
        # str_title = str(key)+"00UTC 28 Jul 2005"
        if key == '00' or key == '03':
            str_title = str(key)+"00UTC 29 Jul"
        else:
            str_title = str(key)+"00UTC 28 Jul"
    ...
    time_list = ['06', '12', '16', '18', '20', '22', '00', '03']
    ...
    fig = plt.figure(figsize=(8, 9), dpi=400)  # 创建页面
    ## 设置子图位置和大小
    ...
    draw_contourf(tbb[5], axes[5], cmap, [title_list[5], '(f)'])
    draw_contourf(tbb[6], axes[6], cmap, [title_list[6], '(g)'])
    cf = draw_contourf(tbb[7], axes[7], cmap, [title_list[7], '(h)'])
