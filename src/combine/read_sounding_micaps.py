...
class GetMicaps():
    ...
    def read_1sta_1time(self, flnm, id=57083):
        ...
        # df3['vertical'] = np.arange(df2['pressure'].shape[0])
        # df2['vertical'] = np.arange(df2['pressure'].shape[0])
        # df3 = df2
        cc = np.arange(df2['pressure'].shape[0])
        df2.insert(0, 'vertical', cc)
        df3 = df2.set_index('vertical')
        # df3 = df3.copy()
        da = xr.DataArray(df3)
        da = da.assign_coords({'time':df.time[0]})
...
def one_station(sta='57083'):
    ...
    path_micaps = '/home/fengx20/project/sod/data_original/obs/Micaps/high/TLOGP/'
    gm = GetMicaps()
    # da = gm.read_1st(path_micaps, 57083)
    da = gm.read_1st(path_micaps, sta)
    save_name = '/home/fengx20/project/sod/data_original/obs/micaps_sounding_station_%s.nc'%sta
...
def dual_station():
    ...
    save_name = '/home/fengx20/project/sod/data_combine/micaps_sounding_station_all.nc'
...
"""
id = 57083
flnm = '/home/fengx20/project/sod/data_original/obs/Micaps/high/TLOGP/20210711080000.000'
df = read_micaps_5(flnm)
ddf = df.loc[df.ID==str(id)]
## 这里是利用read_micaps_5, 只能是这些名称
df1 = ddf.loc[:,['pressure', 'height', 'temperature', 'dewpoint', 'wind_angle', 'wind_speed']]
