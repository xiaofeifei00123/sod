...
def resolveWavesHayashi( varfft: xr.DataArray, nDayWin: int, spd: int ) -> xr.DataArray:  
    ...
    freq      = np.linspace(-1*nDayWin*spd/2, nDayWin*spd/2, (nDayWin*spd)+1)
    ...
    print("++++++++++++++++++++++++++++++++")
    print('nDayWin是%s'%str(nDayWin))
    print('spd是%s'%str(spd))
    aa = np.linspace(-1*nDayWin*spd/2, nDayWin*spd/2, (nDayWin*spd)+1) / nDayWin
    print('自己算的最大值是%s'%str(aa.max()))
    print('freq最大值是%s'%str(freq.max()))
    print('pee_frequency最大值是%s'%str(pee['frequency'].max().values))
    print('ocoords frequency最大值是%s'%str(ocoords['frequency'].max()))
    print("-------------------------------")
    return pee 
...
def split_hann_taper(series_length, fraction):
    ...
    print('数据的时间长度')
    print(npts, fraction, series_length)
    print('数据的时间长度')
...
def spacetime_power(data, segsize=96, noverlap=60, spd=1, latitude_bounds=None, dosymmetries=False, rmvLowFrq=False):
    ...
    # filter low-frequencies  滤除低频率的波  
    if rmvLowFrq:
    ...
    # (time,lat, lon) (730, 13, 144)
    print("****过滤后的data的shape是")
    print(data.shape)
    # logging.info(f"Data reduced by latitude bounds. Size is {data.sizes}")
    ...
    print("****x_wintap的shape是")
    print(x_wintap.shape)
    print(x_wintap.dims)
    ...
    z = xr.DataArray(z, dims=("time","lat","wavenumber","frequency"), 
                     coords={"time":x_wintap["time"], 
                            ...
                            #  "frequency":np.fft.fftfreq(segsize, 1/spd)})
                             "frequency":np.fft.fftfreq(segsize, 1/segsize)})
    print("*******start*****"*5)
    # print(np.fft.fftfreq(segsize,1/spd))
    print(z['frequency'].max(), np.fft.fftfreq(144, 1/144).max())
    # print(z['wavenumber'].max(), np.fft.fftfreq(79, 1/79))
    print("*******end****"*5)
    #
    ...
    print("*******start--z_pee*****"*5)
    # print(np.fft.fftfreq(segsize,1/spd))
    print(z_pee['frequency'].max().values)
    # print(z['wavenumber'].max(), np.fft.fftfreq(79, 1/79))
    print("*******end--z_pee****"*5)
